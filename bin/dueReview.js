...
const dueReview = async () => {
    ...
    mongoose.connect('mongodb://riboadmin:233586364Mgb.@ds249035.mlab.com:49035/ribo', {
    auto_reconnect:true,
    useNewUrlParser: true
    })
    .then(() => { console.log(`Connected to Mongo on ${'mongodb://riboadmin:233586364Mgb.@ds249035.mlab.com:49035/ribo'}`)})
    .then(() => LoanSchedule.updateMany(queryPending, updateToDue))
    .then( objList => console.log({Updated: objList}))
    .then( async (e) => {
        await LoanSchedule.updateMany(queryDue, updateToOverdue).then( resp => console.log({updated: resp}))})   
    .then( async () => {
        await Loan.find(queryClosedLoans).select({ "status": 1, "_id": 1, '_borrower': 0, 'loanSchedule': 0, 'investors': 0})

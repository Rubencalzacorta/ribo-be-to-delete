...
const MonthlyLoanScheduleTotals = (currency) => {
  let c = currency
  return LoanSchedule.find({
      date: {
        $gte: moment().startOf('month'),
        $lte: moment().endOf('month')
      },
      status: {
        $nin: ['DISBURSTMENT', 'CLOSED']
      },
      currency: c
    })
    .then(async data => {
      let totalInterestPaid = await data.reduce((acc, e) => {
        return acc + e.interest_pmt
      }, 0)
      let totalPrincipalPaid = await data.reduce((acc, e) => {
        return acc + e.principal_pmt
      }, 0)
      let totalInterest = await data.reduce((acc, e) => {
        return acc + e.interest
      }, 0)
      let totalPrincipal = await data.reduce((acc, e) => {

...
const loanCrud = (Model, extensionFn) => {
    ...
    router.post('/create', (req, res, next) => {
        ...
        Loan.create({
            ...
            .then(obj => {
                let loanId = obj._id
                let investments = toInvest.map(e => ({
                    _loan: loanId,
                    ...e,
                    currency
                }))
    ...
    router.patch('/installmentpmt/:id', (req, res, next) => {
        ...
        LoanSchedule.findByIdAndUpdate(id, updates, {
                new: true
            })
            .then(obj => {
    ...
    router.post('/restructure/:loanId', async (req, res, next) => {
        ...
        let newLS = await LoanSchedule.find({
            _loan: loanId
        })
        try {
            res.status(200).json({
    ...
    router.delete('/deletepmt/:id', (req, res, next) => {
        ...
        LoanSchedule.findById(id).select({
                "date": 1
            })
            .then(resp => {
    ...
    router.get('/portfolio-status/:country/:fromDate/:toDate', async (req, res, next) => {
        ...
        if (country === 'GLOBAL') {
            ...
            outstandingQuerySearch = await LoanSchedule.aggregate(countryOutstandingQuery(country))
        ...
        Promise.all([allLoansSearch, paidQuerySearch, dueQuerySearch, overdueQuerySearch, outstandingQuerySearch])
            .then(objList => {
                ...
                let unique1 = objList[0].map(e => {
                    return (e._id).toString()
                })
                let unique2 = objList[2].map(e => {
                    return (e._id).toString()
                })
                ...
                let periodDetails = {
                    portfolio: {
                        interest: objList[0].reduce((acc, e) => {
                            return acc + e.interest
                        }, 0),
                        principal: objList[0].reduce((acc, e) => {
                            return acc + e.principal
                        }, 0),
                        interest_pmt: objList[0].reduce((acc, e) => {
                            return acc + e.interest_pmt
                        }, 0),
                        principal_pmt: objList[0].reduce((acc, e) => {
                    ...
                    paid: {
                        interest: objList[1].reduce((acc, e) => {
                            return acc + e.interest
                        }, 0),
                        principal: objList[1].reduce((acc, e) => {
                            return acc + e.principal
                        }, 0),
                        interest_pmt: objList[1].reduce((acc, e) => {
                            return acc + e.interest_pmt
                        }, 0),
                        principal_pmt: objList[1].reduce((acc, e) => {
                            return acc + (e.principal_pmt ? e.principal_pmt : 0)
                        }, 0),
                    ...
                    due: {
                        interest: objList[2].reduce((acc, e) => {
                            return acc + e.interest
                        }, 0),
                        principal: objList[2].reduce((acc, e) => {
                            return acc + e.principal
                        }, 0),
                        interest_pmt: objList[2].reduce((acc, e) => {
                            return acc + e.interest_pmt
                        }, 0),
                        principal_pmt: objList[2].reduce((acc, e) => {
                            return acc + (e.principal_pmt ? e.principal_pmt : 0)
                        }, 0),
                    ...
                    overdue: {
                        interest: objList[3].reduce((acc, e) => {
                            return acc + e.interest
                        }, 0),
                        principal: objList[3].reduce((acc, e) => {
                            return acc + e.principal
                        }, 0),
                        interest_pmt: objList[3].reduce((acc, e) => {
                            return acc + e.interest_pmt
                        }, 0),
                        principal_pmt: objList[3].reduce((acc, e) => {
                            return acc + (e.principal_pmt ? e.principal_pmt : 0)
                        }, 0),
    ...
    router.patch('/update-due', (req, res, next) => {
        ...
        LoanSchedule.updateMany(queryPending, updateToDue)
            .then((e) => {
                ...
                LoanSchedule.updateMany(queryDue, updateToOverdue)
            }).then(resp => console.log({
                updated: resp
            }))
            .catch(e => console.log(e))

        Loan.find(queryClosedLoans).select({
                "status": 1,
                "_id": 1,
                '_borrower': 0,
                'loanSchedule': 0,
    ...
    router.patch('/update-status-database', async (req, res, next) => {
        ...
        await LoanSchedule.updateMany(query, update1).then(resp => console.log({
            updated1: resp
        }))
    })
    ...
    router.patch('/update-database', async (req, res, next) => {
        ...
        LoanSchedule.updateMany({}, {
            ...
            .then(async () => {
                ...
                await Loan.updateMany(query, update1).then(resp => console.log({
                    updated2: resp
                }))
